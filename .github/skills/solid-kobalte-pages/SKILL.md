...
description: "Astro + SolidJS template page construction workflow using Kobalte primitives, lucide-solid icons, Tabler brand icons, IBM Plex Sans, and existing template UI style inheritance. Use for creating, editing, redesigning, or extending pages in this repository."
argument-hint: "ページ目的、作業種別（新規作成/編集/再設計/拡張）、必要セクション、インタラクション要件を指定"
---
...
## When To Use
- 新しいページを追加するとき
- 既存ページをテンプレート様式に沿って編集するとき
- 既存ページを再設計・再構成するとき
- 既存ページへ新しいセクションや機能を拡張するとき

## Out of Scope
- ページ構築に直接関係しない作業
- ページ以外の領域（例: ビルド設定、CI、バックエンド処理）の変更を主目的とする作業
...
- 「既存のTemplateShowcaseを、同じUIトーンを維持したまま情報設計だけ編集して」
- 「既存ページのインタラクティブ部分をKobalteに寄せてアクセシビリティを改善して」
- 「既存のFAQページを再設計して、セクション構造を見直して」
- 「既存ページに比較表セクションを追加して、状態切替はSolid islandで実装して」
